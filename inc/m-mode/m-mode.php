...
function iexcel_mmode_enqueue_scripts() {
...
add_action( 'wp_enqueue_scripts', 'iexcel_mmode_enqueue_scripts' );

/*-----------------------------------------------------------------------------------*/	
/* maintanance mode */
/*-----------------------------------------------------------------------------------*/	
if ( ! function_exists( 'iexcel_maintenance_mode' ) ) {
	function iexcel_maintenance_mode( ) {
		
		$mmode_status = get_theme_mod('mmode_status', 0);
		$iexcel_logo = get_theme_mod( 'logo_trans', '' );
		...
		if( $iexcel_logo ) {
			$maintenance_mode_logo = '<div class="mm-logo-box"><img src="'.esc_url($iexcel_logo).'" alt="'. esc_attr( get_bloginfo( 'name', 'display' ) ).'" style="max-width: 240px;" /></div>';
		} else {
		...
		$maintenance_html .= '<h1>'.esc_html(get_theme_mod('mmode_title', esc_attr__( 'Under Maintenance', 'i-excel' ))).'</h1>';
		$maintenance_html .= '<p>'.esc_html(get_theme_mod('mmode_desc', esc_attr__( 'We are currently in maintenance mode. Please check back shortly.', 'i-excel' ))).'</p>';
	...
	add_action('wp_footer', 'iexcel_maintenance_mode');
}	


add_action('admin_notices', 'iexcel_admin_notice_mmode');
function iexcel_admin_notice_mmode() {

	$mmode_status = get_theme_mod('mmode_status', 0);
    
	if ( $mmode_status == 1 ) {
        echo '<div class="updated mmode-notice"><div style="line-height: 20px;">'; 
		echo esc_html__('Your site is in &quot;Maintanance Mode&quot;. To deactivate the maintanance go to menu &quot;Appearance&quot; &gt; &quot;Customize&quot; &gt; &quot;Coming Soon/Maintenance Mode&quot;.', 'i-excel');
        echo "</div></div>";
    }
}